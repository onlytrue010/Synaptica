...
const FEATURES = [
  { icon: BarChart3, title: '7-Dimension Ratings',      desc: 'Every algorithm rated on accuracy, speed, scalability, interpretability, robustness, ease of use, and data efficiency.', color: 'amber' },
  { icon: Brain,     title: 'Interactive Visualizations',desc: '68+ live, tweakable visualizations. Drag data points, tune parameters, watch models react in real-time.',              color: 'cyan'  },
  { icon: Trophy,    title: '1000+ Interview Q&A',       desc: 'Conceptual, coding, tricky, system design — tagged by company (Google, Meta, Amazon) and difficulty.',                  color: 'emerald'},
  { icon: Code2,     title: 'Code Playground',           desc: 'Write and run Python directly in your browser. No server. No setup. Full scikit-learn and numpy support.',              color: 'purple' },
  { icon: BookOpen,  title: 'Structured Learning Paths', desc: 'Four paths from Beginner to Expert. XP system, streaks, and bookmarks track your progress.',                            color: 'rose'  },
  { icon: Zap,       title: 'Algorithm Compare Tool',    desc: 'Side-by-side radar chart comparison of up to 4 algorithms across all 7 performance dimensions simultaneously.',         color: 'amber' },
]
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f, i) => (
              <Reveal key={f.title} delay={i * 60}>
                <div
                  className="p-6 rounded-xl border transition-all duration-300 hover:-translate-y-1 cursor-default"
                  style={{ background: 'var(--color-card-bg)', borderColor: 'var(--color-border)' }}
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-4 ${COLOR_MAP[f.color]}`}>
                    <f.icon size={18} />
                  </div>
                  <h3 className="text-sm font-medium mb-2" style={{ color: 'var(--color-text-1)' }}>
                    {f.title}
                  </h3>
